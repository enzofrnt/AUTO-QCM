...
from app.models import QCM, Question, ReponseQCM, ReponseQuestion, Utilisateur
...
def qcm_statistics(request, pk=None):
    ...
    qcms = (
        ...
        .distinct()
        .prefetch_related("reponses_qcm", "questions")
    )
    qcms_stats = []
    ...
    for qcm in qcms:
        ...
        if total_responses > 0:
            ...
            for reponse in reponses:
                max_score = (
                    reponse.score_max if reponse.score_max > 0 else 1
                )  # Éviter la division par zéro
                percentage = (reponse.score / max_score) * 100
                total_percentage += percentage

            avg_score_percentage = (
                total_percentage / total_responses
        ...
        questions_stats = []
        question_scores = {
            question.id: {"total_score": 0, "total_responses": 0}
            ...
            for reponse_question in reponse_qcm.reponses.all():
                ...
                if question_id in question_scores:
                    ...
                    question_scores[question_id][
                        "total_score"
                    ] += reponse_question.score  # Corrected line
        ...
        for question in qcm.questions.all():
            ...
            if total_question_responses > 0:
                max_question_score = (
                    question.note if question.note > 0 else 1
                )  # Éviter la division par zéro
                avg_question_percentage = (
                    question_scores[question.id]["total_score"]
                    / (max_question_score * total_question_responses)
                ) * 100
            else:
